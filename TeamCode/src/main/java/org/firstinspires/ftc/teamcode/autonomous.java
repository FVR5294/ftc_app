...
public class autonomous extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        p.move(Math.abs(measurements.tileLength - measurements.robotDepth) / 2, 0, 0, robot, telemetry);
        p.move(measurements.tileLength, color * measurements.tileLength * -1, 0, robot, telemetry);
        p.move(0, 0, color * -90, robot, telemetry);
        p.move(0, color * measurements.tileLength, 0, robot, telemetry);
        p.move(measurements.tileLength, 0, 0, robot, telemetry);
        ...
        p.move(measurements.mmPerInch * -4, 0, 0, robot, telemetry); //back away from the button
        robot.pushButton(0);//reset button pusher to prevent accidental press at next beacon
        p.move(0, color * measurements.tileLength * 2, 0, robot, telemetry);//slide to the right/left
        p.move(measurements.mmPerInch * 4.01, 0, 0, robot, telemetry); //get next button

        robot.pushButton(robot.detectColor() * color);//admit it, my way is better than your's

        p.move(-(measurements.tileLength - measurements.robotDepth) / 2, 0, 0, robot, telemetry);

        p.move(-measurements.tileLength * 2, -measurements.tileLength * color * 2, 0, robot, telemetry);
