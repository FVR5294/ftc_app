...
//@Autonomous(name = "master v2", group = "above")

public class masterV2 extends LinearOpMode {
    ...
    private List<List<state>> chooselist = Arrays.asList(Arrays.asList(states.colorBlue, states.colorRed), Arrays.asList(states.sleep0, states.sleep1000, states.sleep4000, states.sleep10000), Arrays.asList(states.arcTowardsBeacon20, states.arcTowardsBeacon24, states.arcTowardsBeacon28, states.arcTowardsBeacon32), Arrays.asList(states.scanForLine), Arrays.asList(states.driveTowardsBeacon, states.sleep0), Arrays.asList(states.pushBeaconButton, states.sleep0), Arrays.asList(states.backAwayFromBeacon10, states.backAwayFromBeacon15, states.backAwayFromBeacon20), Arrays.asList(states.shootball, states.shootball2), Arrays.asList(states.sleep0, states.sleep500, states.sleep1000, states.sleep2000, states.sleep4000, states.noscope), Arrays.asList(states.shootball, states.shootball2), Arrays.asList(states.correctStrafe4, states.correctStrafe8, states.correctStrafe12, states.correctStrafe16, states.slideToTheRight50, states.slideToTheRight54, states.slideToTheRight58), Arrays.asList(states.correctStrafe4, states.correctStrafe8, states.correctStrafe12, states.correctStrafe16, states.slideToTheRight50, states.slideToTheRight54, states.slideToTheRight58), Arrays.asList(states.scanForLine, states.sleep0), Arrays.asList(states.driveTowardsBeacon, states.sleep0), Arrays.asList(states.pushBeaconButton, states.sleep0), Arrays.asList(states.pivotbeaconless, states.pivotbeacon, states.pivotbeaconmore), Arrays.asList(states.backuptovortexReduced, states.backuptovortex, states.backuptovortexIncreased, states.backup84));
...
//                    int choiceindex = Math.max(chooselist.get(selectedstate).indexOf(list.get(selectedstate)), 0);
//                    //add copy of current state after the current state
//                    list.add(selectedstate + 1, chooselist.get(selectedstate).get(choiceindex));
//                    chooselist.add(selectedstate + 1, chooselist.get(selectedstate));
...
//                    chooselist.remove(selectedstate);
//                    selectedstate++;
//                    selectedstate = list.size() + selectedstate;
//                    selectedstate = selectedstate % list.size();
//                }
                ...
                if (gamepad1.dpad_left) {
                    ...
                    int choiceindex = (chooselist.size() + chooselist.get(selectedstate).indexOf(list.get(selectedstate)) - 1) % chooselist.get(selectedstate).size();
                    //delete selected state
                    list.remove(selectedstate);
                    //replace selected state with the item before it in the choice list
                    list.add(selectedstate, chooselist.get(selectedstate).get(choiceindex));
